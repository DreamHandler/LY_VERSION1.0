...
(function ($)
{
    ...
    $.LyDialog = function ()
    {
        return l.run.call(null, "LyDialog", arguments, { isStatic: true });
    };

    //dialog 图片文件夹的路径 预加载
    $.Lyui.DialogImagePath = "../skins/default/images/win/";
    ...
    $.LyMethos.Dialog = $.LyMethos.Dialog || {};
    ...
    l.controls.Dialog.LyExtend(l.core.Win, {
        __getType: function ()
        {
            return 'Dialog';
        },
        ...
        _render: function ()
        {
            ...
            else if (p.url)
            {
                ...
                else
                {
                    ...
                    var framename = p.name ? p.name : "Lywindow" + new Date().getTime();
                    ...
                    // 加入以下代码 
                    tmpId = 'jquery_Lyui_' + new Date().getTime();
        ...
        show: function ()
        {
            ...
            //前端显示 
            $.Lyui.win.setFront.LyDefer($.Lyui.win, 100, [g]);
        },
        setUrl: function (url)
        {
            this._setUrl(url);
            ...
            if ($.fn.LyDrag)
            {
                g.draggable = g.dialog.LyDrag({
                    ...
                    onStopDrag: function ()
                    {
                        ...
                        if (p.target)
                        {
                            var triggers1 = l.find($.Lyui.controls.DateEditor);
                            var triggers2 = l.find($.Lyui.controls.ComboBox);
                            //更新所有下拉选择框的位置
        ...
        _onReisze: function ()
        {
            ...
            if (p.target || p.url)
            {
                var manager = $(p.target).Ly();
                if (!manager) manager = $(p.target).find(":first").Ly();
                if (!manager) return;
        ...
        _applyResize: function ()
        {
            ...
            if ($.fn.LyResizable)
            {
                g.resizable = g.dialog.LyResizable({
    ...
    $.LyDialog.open = function (p)
    {
        return $.LyDialog(p);
    };
    $.LyDialog.close = function ()
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        for (var i in dialogs)
        {
            var d = dialogs[i];
            d.destroy.LyDefer(d, 5);
        }
        l.win.unmask();
    };
    $.LyDialog.show = function (p)
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        if (dialogs.length)
        {
    ...
    $.LyDialog.tip = function (options)
    {
        ...
        return $.LyDialog.open(options);
    };
    $.LyDialog.alert = function (content, title, type, callback, options)
    {
        content = content || "";
        ...
        p = {
            content: content,
            buttons: [{ text: $.LyDefaults.DialogString.ok, onclick: btnclick }]
        };
    ...
    $.LyDialog.confirm = function (content, title, callback)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            ...
            buttons: [
                {
                    text: $.LyDefaults.DialogString.yes, onclick: btnclick, type: 'ok', cls: 'l-dialog-btn-ok'
                }, {
                    text: $.LyDefaults.DialogString.no, onclick: btnclick, type: 'no', cls: 'l-dialog-btn-no'
                }
            ]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
    ...
    $.LyDialog.warning = function (content, title, callback, options)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.LyDefaults.DialogString.yes, onclick: btnclick, type: 'yes' }, { text: $.LyDefaults.DialogString.no, onclick: btnclick, type: 'no' }, { text: $.LyDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.LyDialog.waitting = function (title)
    {
        title = title || $.LyDefaults.Dialog.waittingMessage;
        return $.LyDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: false });
    };
    $.LyDialog.closeWaitting = function ()
    {
    ...
    $.LyDialog.success = function (content, title, onBtnClick, options)
    {
        return $.LyDialog.alert(content, title, 'success', onBtnClick, options);
    };
    $.LyDialog.error = function (content, title, onBtnClick, options)
    {
        return $.LyDialog.alert(content, title, 'error', onBtnClick, options);
    };
    $.LyDialog.warn = function (content, title, onBtnClick, options)
    {
        return $.LyDialog.alert(content, title, 'warn', onBtnClick, options);
    };
    $.LyDialog.question = function (content, title, options)
    {
        return $.LyDialog.alert(content, title, 'question', null, options);
    };


    $.LyDialog.prompt = function (title, value, multi, callback)
    {
        ...
        p = {
            ...
            target: target,
            width: 320,
            buttons: [{ text: $.LyDefaults.DialogString.ok, onclick: btnclick, type: 'yes' }, { text: $.LyDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        return $.LyDialog(p);
    };
