InitCompriseCss(BassModuleUrl+"Dialog/Css/Aqua/css/yxui-dialog.css");
InitCompriseCss(BassModuleUrl+"Dialog/Css/Aqua/css/green.css");
InitComprise(BassModuleUrl+"Dialog/jQMessageBox.js");
/**
* jQuery YxUI 1.3.3
* 
* http://Yxui.com
...
(function ($)
{
    ...
    $.YxDialog = function ()
    {
        return l.run.call(null, "YxDialog", arguments, { isStatic: true });
    };

    //dialog 图片文件夹的路径 预加载
    $.Yxui.DialogImagePath = "../Css/skins/Aqua/images/win/";
    ...
    $.YxMethos.Dialog = $.YxMethos.Dialog || {};
    ...
    l.controls.Dialog.YxExtend(l.core.Win, {
        __getType: function ()
        {
            return 'Dialog';
        },
        ...
        _render: function ()
        {
            ...
            else if (p.url)
            {
                ...
                else
                {
                    ...
                    var framename = p.name ? p.name : "Yxwindow" + new Date().getTime();
                    ...
                    // 加入以下代码 
                    tmpId = 'jquery_Yxui_' + new Date().getTime();
        ...
        show: function ()
        {
            ...
            //前端显示 
            $.Yxui.win.setFront.YxDefer($.Yxui.win, 100, [g]);
        },
        setUrl: function (url)
        {
            this._setUrl(url);
            ...
            if ($.fn.YxDrag)
            {
                g.draggable = g.dialog.YxDrag({
                    ...
                    onStopDrag: function ()
                    {
                        ...
                        if (p.target)
                        {
                            var triggers1 = l.find($.Yxui.controls.DateEditor);
                            var triggers2 = l.find($.Yxui.controls.ComboBox);
                            //更新所有下拉选择框的位置
        ...
        _onReisze: function ()
        {
            ...
            if (p.target || p.url)
            {
                var manager = $(p.target).Yx();
                if (!manager) manager = $(p.target).find(":first").Yx();
                if (!manager) return;
        ...
        _applyResize: function ()
        {
            ...
            if ($.fn.YxResizable)
            {
                g.resizable = g.dialog.YxResizable({
    ...
    $.YxDialog.open = function (p)
    {
        return $.YxDialog(p);
    };
    $.YxDialog.close = function ()
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        for (var i in dialogs)
        {
            var d = dialogs[i];
            d.destroy.YxDefer(d, 5);
        }
        l.win.unmask();
    };
    $.YxDialog.show = function (p)
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        if (dialogs.length)
        {
    ...
    $.YxDialog.tip = function (options)
    {
        ...
        return $.YxDialog.open(options);
    };
    $.YxDialog.alert = function (content, title, type, callback, options)
    {
        content = content || "";
        ...
        p = {
            content: content,
            buttons: [{ text: $.YxDefaults.DialogString.ok, onclick: btnclick }]
        };
    ...
    $.YxDialog.confirm = function (content, title, callback)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            ...
            buttons: [
                {
                    text: $.YxDefaults.DialogString.yes, onclick: btnclick, type: 'ok', cls: 'l-dialog-btn-ok'
                }, {
                    text: $.YxDefaults.DialogString.no, onclick: btnclick, type: 'no', cls: 'l-dialog-btn-no'
                }
            ]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
    ...
    $.YxDialog.warning = function (content, title, callback, options)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.YxDefaults.DialogString.yes, onclick: btnclick, type: 'yes' }, { text: $.YxDefaults.DialogString.no, onclick: btnclick, type: 'no' }, { text: $.YxDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.YxDialog.waitting = function (title)
    {
        title = title || $.YxDefaults.Dialog.waittingMessage;
        return $.YxDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: false });
    };
    $.YxDialog.closeWaitting = function ()
    {
    ...
    $.YxDialog.success = function (content, title, onBtnClick, options)
    {
        return $.YxDialog.alert(content, title, 'success', onBtnClick, options);
    };
    $.YxDialog.error = function (content, title, onBtnClick, options)
    {
        return $.YxDialog.alert(content, title, 'error', onBtnClick, options);
    };
    $.YxDialog.warn = function (content, title, onBtnClick, options)
    {
        return $.YxDialog.alert(content, title, 'warn', onBtnClick, options);
    };
    $.YxDialog.question = function (content, title, options)
    {
        return $.YxDialog.alert(content, title, 'question', null, options);
    };


    $.YxDialog.prompt = function (title, value, multi, callback)
    {
        ...
        p = {
            ...
            target: target,
            width: 320,
            buttons: [{ text: $.YxDefaults.DialogString.ok, onclick: btnclick, type: 'yes' }, { text: $.YxDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        return $.YxDialog(p);
    };
