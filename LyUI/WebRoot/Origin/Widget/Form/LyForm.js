InitCompriseCss("/LyUI/Origin/Widget/skins/default/css/Lyui-form.css");
/**
* jQuery LyUI 1.3.3
* 
* http://Lyui.com
...
(function ($)
{
    ...
    $.fn.LyForm = function ()
    {
        return $.Lyui.run.call(this, "LyForm", arguments);
    };

    $.Lyui.getConditions = function (form, options)
    {
        if (!form) return null;
        form = Ly.get($(form));
    ...
    $.LyDefaults.Form = {
        ...
        //字段
        /*
        数组的集合,支持的类型包括在$.LyDefaults.Form.editors,这个editors同Grid的editors继承于base.js中提供的编辑器集合,具体可以看Ly.editors
        字段的参数参考 127行左右的 $.LyDefaults.Form_fields,
        ui内置的编辑表单元素都会调用ui的表单插件集合,所以这些字段都有属于自己的"Ly对象",可以同Ly.get("[ID]")的方式获取，这里的[ID]获取方式优先级如下：
        ...
        */
        ...
        buttons: null,              //按钮组
        readonly: false,              //是否只读
        editors: {},              //编辑器集合,使用同$.LyDefaults.Grid.editors
        //验证
        validate: null,
    ...
    $.LyDefaults.FormString = {
        invalidMessage: '存在{errorCount}个字段验证不通过，请检查!',
        detailMessage: '详细',
        okMessage: '确定'
    };

    $.LyDefaults.Form.editors.textarea =
    {
    ...
    $.LyDefaults.Form.editors.hidden =
    {
        create: function (container, editParm, p)
        {
    ...
    $.LyDefaults.Form.editorBulider = function (jinput)
    {
        //这里this就是form的Lyui对象
        ...
        {
            control = jinput.LyComboBox(options);
        }
        else if (jinput.is(":password") && ltype != "none")
        {
            control = jinput.LyTextBox(options);
        } 
        else if (jinput.is(":radio") && ltype != "none")
        {
            control = jinput.LyRadio(options);
        }
        else if (jinput.is(":checkbox") && ltype != "none")
        {
            control = jinput.LyCheckBox(options);
        }
            ...
            if (jinput.is(":text"))
            {
                switch (ltype)
                {
                    ...
                    case "autocomplete":
                        ...
                        break;
                    case "spinner":
                        control = jinput.LySpinner(options);
                        break;
                    case "date":
                        control = jinput.LyDateEditor(options);
                        break;
                    case "popup":
                        control = jinput.LyPopupEdit(options);
                        break;
                    case "currency":
            ...
            else if (jinput.is(":hidden"))
            {
                switch (ltype)
                {
                    ...
                    case "autocomplete":
                        ...
                        break;
                    case "checkboxlist":
                        control = jinput.LyCheckBoxList(options);
                        break;
                    case "radiolist":
                        control = jinput.LyRadioList(options);
                        break;
                    case "listbox":
                        control = jinput.LyListBox(options);
                        break; 
                }
            }
        } 
    ...
    //表单组件
    $.Lyui.controls.Form = function (element, options)
    {
        $.Lyui.controls.Form.base.constructor.call(this, element, options);
    };

    $.Lyui.controls.Form.LyExtend($.Lyui.core.UIComponent, {
        __getType: function ()
        {
            return 'Form'
        },
        ...
        {
            var g = this, p = this.options;
            $.Lyui.controls.Form.base._init.call(this);
            //编辑构造器初始化
        ...
        _render: function ()
        {
            ...
            if (p.buttons)
            {
                ...
                $(p.buttons).each(function ()
                {
                    ...
                    $("div:first", jbutton).LyButton(this);
                });
            }
        ...
        showFieldError: function (name, errorText)
        {
            ...
            $(element).removeAttr("title").LyHideTip();
            $(element).attr("title", errorText).LyTip({
                distanceX: 5,
                distanceY: -3,
                auto: true
            });
        ...
        initValidate: function ()
        {
            ...
            var validateOptions = $.extend({
                errorPlacement: function (lable, element)
                {
                    ...
                    $(element).removeAttr("title").LyHideTip();
                    $(element).attr("title", $(lable).html()).LyTip({
                        distanceX: 5,
                        distanceY: -3,
                        auto: true
                    });
        ...
        setFieldValidate: function (name, validate, messages)
        {
            ...
            if (oldRule) //旧的验证移除验证结果
            {
                ...
                jele.removeAttr("title").LyHideTip();
