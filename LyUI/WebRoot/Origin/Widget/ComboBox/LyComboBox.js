...

{
    ...
    $.fn.LyComboBox = function (options)

    {

        return $.Lyui.run.call(this, "LyComboBox", arguments);

    };

    $.fn.LyGetComboBoxManager = function ()

    {

        return $.Lyui.run.call(this, "LyGetComboBoxManager", arguments);
    ...
    $.LyDefaults.ComboBox = {
        ...
        tree: null,            //下拉框以树的形式显示，tree的参数跟LyTree的参数一致 
        treeLeafOnly: true,   //是否只选择叶子
        condition: null,       //列表条件搜索 参数同 LyForm
        grid: null,              //表格 参数同 LyGrid
        onStartResize: null,
        onEndResize: null,
    ...
    //扩展方法
    $.LyMethos.ComboBox = $.LyMethos.ComboBox || {};


    $.Lyui.controls.ComboBox = function (element, options)

    {

        $.Lyui.controls.ComboBox.base.constructor.call(this, element, options);

    };
    $.Lyui.controls.ComboBox.LyExtend($.Lyui.controls.Input, {
...

        {
            ...
            $.Lyui.controls.ComboBox.base._init.call(this);
            var p = this.options;
            if (p.columns)
...

        {
            ...
            g.valueField.attr("data-Lyid", g.id);
            //开关
...

        {
            ...
            //调整大小支持
            if (resize && $.fn.LyResizable)

            {

                var handles = p.selectBoxHeight ? 'e' : 'se,s,e';
                g.selectBox.LyResizable({
...

{
...

        {
            ...
            g.set('selectBoxHeight', p.selectBoxHeight);
            //自定义复选框支持
            if (p.isShowCheckBox && $.fn.LyCheckBox)

            {

                $("table input:checkbox", g.selectBox).LyCheckBox();
...

        {
            ...
            //新增下拉框中获取树对象的接口
            g.innerTree = g.tree.LyTree(tree);
            g.treeManager = g.tree.LyGetTreeManager();
...

        {
...

            {
                ...
                g.condition = conditionPanel.LyForm(conditionParm);
            ...
            g.grid = g.gridManager = gridPanel.LyGrid(grid);
...

                    {
...

                        {
...

                            {
                                ...
                                g.grid.setParm(grid.conditionParmName || 'condition', $.Lyui.toJSON(rules));
                                g.grid.reload();
...

                            {
                                ...
                                g.grid.loadData($.LyFilter.getFilterFunction(rules));
...

{
...

        {
            ...
            if (!g || !p) return;
            //避免同一界面弹出多个菜单的问题
            var managers = $.Lyui.find($.Lyui.controls.ComboBox);
            ...
            managers = $.Lyui.find($.Lyui.controls.DateEditor);
            for (var i = 0, l = managers.length; i < l; i++)
    ...
    $.Lyui.controls.ComboBox.prototype.setValue = $.Lyui.controls.ComboBox.prototype.selectValue;
    //设置文本框和隐藏控件的值
    $.Lyui.controls.ComboBox.prototype.setInputValue = $.Lyui.controls.ComboBox.prototype._changeValue;
    ...
    (function ()
    {
        $(document).unbind('keydown.Lycombobox');
        $(document).bind('keydown.Lycombobox',function (event)
        {
            ...
            { 
                var combobox = Ly.get(curInput.attr("data-comboboxid"));
                if (!combobox) return;
