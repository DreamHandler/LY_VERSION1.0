...
(function ($)
{
    ...
    $.fn.LyDrag = function (options)
    {
        return l.run.call(this, "LyDrag", arguments,
        {
            idAttrName: 'Lyuidragid', hasElement: false, propertyToElemnt: 'target'
        }
        );
    };

    $.fn.LyGetDragManager = function ()
    {
        return l.run.call(this, "LyGetDragManager", arguments,
        {
            idAttrName: 'Lyuidragid', hasElement: false, propertyToElemnt: 'target'
        });
    };
    ...
    $.LyDefaults.Drag = {
        ...
        onDrop: null,
        disabled: false,
        proxLy: null,     //代理相对鼠标指针的位置,如果不设置则对应target的left
        proxyY: null
    };
    ...
    l.controls.Drag.LyExtend(l.core.UIComponent, {
        __getType: function ()
        {
            return 'Drag';
        },
        ...
        _rendered: function ()
        {
            this.options.target.Lyuidragid = this.id;
        },
        _start: function (e)
        {
            var g = this, p = this.options;
        ...
        _applyDrag: function (applyResultBody)
        {
            ...
            if (g.current.diffX)
            {
                if (noproxy || p.proxLy == null)
                    cur.left = g.current.left + g.current.diffX;
                else
                    cur.left = g.current.startX + p.proxLy + g.current.diffX;
                changed = true;
            }
            if (g.current.diffY)
            {
        ...
        _createProxy: function (proxy, e)
        {
            ...
            g.proxy.css({
                left: p.proxLy == null ? g.current.left : g.current.startX + p.proxLy,
